/**
 * @file Gestisce la visualizzazione dei dettagli di un singolo spazio di lavoro,
 * la gestione delle prenotazioni, la visualizzazione di recensioni, e l'interazione con la mappa.
...
/**
 * Funzione di utilità per selezionare un elemento del DOM.
 * @param {string} s - Il selettore CSS.
 * @returns {Element|null} L'elemento del DOM corrispondente al selettore.
 */
const $ = (s) => document.querySelector(s);
/** @type {URLSearchParams} params - I parametri URL della pagina. */
const params = new URLSearchParams(location.search);
/** @type {string|null} spazioId - L'ID dello spazio di lavoro ottenuto dall'URL. */
const spazioId = params.get("id");

// fasce orarie
/**
 * Array di oggetti che definiscono le fasce orarie disponibili.
 * @type {Array<Object>}
 */
const FASCE = [
...
/** @type {string} valuta - La valuta di default per i prezzi. */
let valuta = "€";
/** @type {object|null} user - L'oggetto utente loggato, recuperato dal localStorage. */
let user = null;
/** @type {Array<number>} selectedSlots - Un array degli ID delle fasce orarie selezionate dall'utente. */
let selectedSlots = [];
/** @type {string|null} gestore_id - L'ID del gestore dello spazio di lavoro. */
let gestore_id = null;
...
/**
 * Imposta il testo di un elemento HTML specificato dall'ID.
 * @param {string} id - L'ID dell'elemento.
 * @param {string} text - Il testo da impostare.
 */
...
/**
 * Carica i dati dello spazio di lavoro dal database e aggiorna la pagina.
 * @async
 */
...
/**
 * Renderizza i servizi dello spazio di lavoro in una lista HTML.
 * @param {Array<string>} servizi - L'array di servizi da visualizzare.
 */
...
/**
 * Renderizza la galleria di immagini dello spazio di lavoro.
 * @param {Array<string>} imgs - L'array di URL delle immagini.
 */
...
/**
 * Inizializza i campi di input per la data con la data odierna.
 */
 ...
 * Disabilita le fasce già prenotate.
 * @async
 */
...
/**
 * Abilita o disabilita il pulsante di call-to-action (CTA) per la prenotazione.
 */
function toggleCTA() {
...
/**
 * Associa un listener al pulsante di prenotazione.
 */
function bindCTA() {
...
/**
 * Carica e visualizza gli spazi di lavoro correlati (altri spazi).
 * @async
 */
...
/**
 * Calcola l'occupazione delle fasce orarie per gli ultimi 30 giorni.
 * @async
 * @returns {Array<Object>} Un array di punti per il grafico sparkline.
 */
...
/**
 * Disegna un grafico sparkline (un piccolo grafico a linea senza assi) che mostra l'occupazione.
 * @param {Array<Object>} points - Un array di punti {x, y} da disegnare.
 */
...
/**
 * Formatta un numero come prezzo con simbolo di valuta.
 * @param {number} n - Il numero da formattare.
 * @param {string} [cur="€"] - Il simbolo della valuta.
 * @returns {string} La stringa del prezzo formattato.
 */
...
/**
 * Carica e visualizza le recensioni per lo spazio di lavoro corrente.
 * Calcola la media dei voti e aggiorna i badge.
 * @async
 */
